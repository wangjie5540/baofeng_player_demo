...
/**
 * 绘制前，renderer的配置，初始化EGL，开始一个绘制线程. 这个类需要子类去实现相应的绘制工作.
 * 
 ...
 */
public abstract class TextureSurfaceRenderer extends HlsRendererBuilder
		implements Runnable {
	...
	public TextureSurfaceRenderer(SurfaceTexture surfaceTexture, int width,
			int height, String angent, String url) {
		super(angent, url);
	...
	/***
	 * 初始化opengl的一些组件比如vertextBuffer,sharders,textures等， 通常在Opengl context
	 * 初始化以后被调用，需要子类去实现
	 */
	...
	private EGLConfig chooseEglConfig() {
		...
		if (!egl.eglChooseConfig(eglDisplay, attributes, configs, confSize,
				configsCount)) { // 获取满足attributes的config个数
	...
	private int[] getAttributes() {
		return new int[] {
				...
				EGL14.EGL_OPENGL_ES2_BIT, // 指定渲染api类别
