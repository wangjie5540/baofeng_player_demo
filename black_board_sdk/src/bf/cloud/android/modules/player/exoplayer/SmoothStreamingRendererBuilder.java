...
public class SmoothStreamingRendererBuilder implements RendererBuilder,
  ...
  @Override
  public void buildRenderers(ExoVideoPlayer player, RendererBuilderCallback callback) {
    this.player = player;
  ...
  public void onSingleManifest(SmoothStreamingManifest manifest) {
    ...
    if (textStreamElementCount == 0) {
      ...
      ChunkSampleSource ttmlSampleSource = new ChunkSampleSource(textChunkSource, loadControl,
          ...
          ExoVideoPlayer.TYPE_TEXT);
      textRenderer = new TextTrackRenderer(ttmlSampleSource, player, mainHandler.getLooper(),
          new TtmlParser());
    }

    // Invoke the callback.
    String[][] trackNames = new String[ExoVideoPlayer.RENDERER_COUNT][];
    trackNames[ExoVideoPlayer.TYPE_AUDIO] = audioTrackNames;
    trackNames[ExoVideoPlayer.TYPE_TEXT] = textTrackNames;

    MultiTrackChunkSource[] multiTrackChunkSources =
        new MultiTrackChunkSource[ExoVideoPlayer.RENDERER_COUNT];
    multiTrackChunkSources[ExoVideoPlayer.TYPE_AUDIO] = audioChunkSource;
    multiTrackChunkSources[ExoVideoPlayer.TYPE_TEXT] = textChunkSource;
    ...
    renderers[ExoVideoPlayer.TYPE_DEBUG] = debugRenderer;
  ...
  private static class V18Compat {
    ...
    public static DrmSessionManager getDrmSessionManager(UUID uuid, ExoVideoPlayer player,
