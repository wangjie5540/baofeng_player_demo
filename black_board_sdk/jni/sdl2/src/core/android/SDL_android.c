...
{
    __android_log_print(ANDROID_LOG_INFO, "SDL", "onNativeResize. w:%d, h:%d", width, height);
	...
	JNIEnv* env, jclass jcls)
{
	...
	SDL_Event event;
	event.type = (SDL_USEREVENT + 7);   // PLAYER_SURFACE_CHANGED_EVENT (in player.cpp)
	SDL_PushEvent(&event);
...
int Android_JNI_SendMessage(int message, int param)
{
    JNIEnv *env = Android_JNI_GetEnv();
    if (!env) {
        return -1;
    ...
    jboolean success = (*env)->CallStaticBooleanMethod(env, mActivityClass, mid, message, param);
    return success ? 0 : -1;
}
